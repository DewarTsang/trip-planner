...
onMounted(async () => {
  ...
  if (data) {
    ...
    // 景点图片通过后端高德POI接口自动获取，存在 image_url / photos 字段中
    // 等待DOM渲染完成后初始化地图
    await nextTick()
    initMap()
  }
...
// 获取景点图片: 优先使用后端返回的高德POI真实图片，否则用SVG渐变占位图
const getAttractionImage = (item: any, index: number): string => {
  // 1. 后端 enrich_images 节点返回的真实图片
  if (item.image_url) return item.image_url
  if (item.photos && item.photos.length > 0) return item.photos[0]

  // 2. SVG 渐变占位图 (国内网络环境下 Unsplash 不可用)
